...
namespace Controllers
{
    ...
    {
        public string DadosVendaDiaria()
        {
            var query = "SELECT SUM(V.Quantidade),SUM(V.Valor_Pago) FROM tbVenda AS V INNER JOIN tbFuncionario AS F ON V.ID_Funcionario=F.id WHERE V.Data_Venda=@Data";
        ...
        public void SaveDivida(int idProd, int IdFunc, int quant, int  VALORPAGAR,int codigo, DateTime data,int preco)
        {
            this.ID_PRODUTO = idProd;
            this.ID_FUNCIONARIO = IdFunc;
            this.QUANTIDADE_VENDA = quant;
            this.VALOR_PAGAR = Convert.ToDouble(VALORPAGAR);
           
           
            //----------------------------------------------------------------------------------------------------------------
            var query = "INSERT INTO tbDividas(ID_Produto,ID_Funcionario,Nome_Cliente,Quantidade,Preco,Valor_Pagar,Codigo,Data_Venda)"
                            + " VALUES(@idprod,@idFunc,@nome,@quant,@preco,@valor,@codigo,@data)";
            //----------------------------------------------------------------------------------------------------------------
            ...
            List<ControllerBD.SQLParametro> parametro = new List<ControllerBD.SQLParametro>() 
                { 
                    ...
                    new ControllerBD.SQLParametro("@nome", this.NOME_CLIENTE),
                    new ControllerBD.SQLParametro("@quant", this.QUANTIDADE_VENDA),
                    new ControllerBD.SQLParametro("@preco", preco),
                    new ControllerBD.SQLParametro("@valor", this.VALOR_PAGAR),
                     new ControllerBD.SQLParametro("@codigo", codigo),
                    new ControllerBD.SQLParametro("@data", Convert.ToDateTime(DateTime.Now.ToShortDateString()))
