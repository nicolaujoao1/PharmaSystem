...
namespace Controllers
{
    public class ControllerStore:ModelStore
    {
        public void InsertStore(){
            ...
            {
                ////--------------------------------------------------------------------------------------
                string query = "INSERT INTO tbArmazem VALUES(@id_funcionario,@nome_medicamento,@descricao,@data_validade,@familia,@fornecedor,@codigo_platileira,@quantidade_entrada,@quantMinima,@preco_unitario,@IdAGT,@unidade,@origem,@codigo_barra)";
                ////--------------------------------------------------------------------------------------
        ...
        public void UpdateStore(string nameSelect)
        {
            try
            {
                ...
                //--------------------------------------------------------------------------------
                var query = "UPDATE tbArmazem SET Nome_Medicamento =@nome, Descricao=@descr,Data_Validade=@data,Familia=@familia,Fornecedor=@fornecedor,Codigo_Platileira=@codPlat,Quantidade_Entrada=@quant,Quantidade_Minima=@quantMinima,Preco_Unitario=@preco,idAgt=@IdAGT,Unidade=@unidade,Origem=@origem,Codigo_Barra=@codbar WHERE Nome_Medicamento=@nomeCod";
                //--------------------------------------------------------------------------------
                List<ControllerBD.SQLParametro> parametro = new List<ControllerBD.SQLParametro>() 
                { 
        ...
        public DataTable SelectStore(string nameSelect) {
            //------------------------------------------------------------------------------
            var query1 = "SELECT A.* FROM tbArmazem As A Inner Join tbAGT on A.IdAgt=tbAGT.id  WHERE Nome_Medicamento=@nom";
            //------------------------------------------------------------------------------
        ...
        {
               var query1 = "SELECT Nome_Medicamento as Medicamento,Quantidade_Entrada as Quantidade,Data_Validade  as Validade,Preco_Unitario as Preço FROM tbArmazem";
