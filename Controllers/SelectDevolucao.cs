   ...
   public class SelectDevolucao
    {
        public static DataTable CarregarDados(int id)
        {
            try
            {
                ...
                var query = "SELECT P.id AS ID,P.Nome_Medicamento AS Medicamento,P.Preco_Unitario AS Preco,V.Nome_Cliente AS Cliente,V.Quantidade,V.Valor_Pago,V.Data_Venda,V.ID_Venda  FROM tbVenda AS V INNER JOIN tbProduto AS P ON P.id=V.ID_Produto WHERE V.Data_Venda=@data";
                //------------------------------------------------------------------------------
