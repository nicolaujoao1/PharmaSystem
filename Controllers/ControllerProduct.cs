...
namespace Controllers
{
    public class ControllerProduct:ModelProduct
    {
        ...
        public int QuantDesejada { get; set; }
         public void InsertProduct(){
                 ...
                 try
                 {
                     ...
                     int v = QuantidadeInserir(this.NOME_MEDICAMENTO);
                     if (QuantidadeInserir(this.NOME_MEDICAMENTO)==0)
                     {
                         Analizar();
                     }
                     else
                     if(QuantidadeInserir(this.NOME_MEDICAMENTO)<this.QUANTIDADE_OUTRO)return;
                     else
                        
                     {
                         ...
                         {
                             ////--------------------------------------------------------------------------------------
                             string query = "INSERT INTO tbProduto VALUES(@id_funcionario,@nome_medicamento,@descricao,@data_validade,@familia,@fornecedor,@codigo_platileira,@quantidade_entrada,@quantMinima,@preco_unitario,@idAgt,@unidade,@origem,@codigo_barra)";
                             ////--------------------------------------------------------------------------------------
                             List<ControllerBD.SQLParametro> parametro = new List<ControllerBD.SQLParametro>() 
                            {
                                ...
                                new ControllerBD.SQLParametro("@familia", this.FAMILA),
                                new ControllerBD.SQLParametro("@fornecedor", this.FORNECEDOR),
                                ...
                                new ControllerBD.SQLParametro("@idAgt", this.ID_AGT),
         ...
         public void UpdateAlgunsProduto(string nameProd)
         {
             try
             {
                 //--------------------------------------------------------------------------------
                 var query = "UPDATE tbProduto SET Nome_Medicamento =@nome, Quantidade_Entrada=@quant,Preco_Unitario=@preco,Codigo_Barra=@codigo WHERE Nome_Medicamento=@nomeCod";
                 //--------------------------------------------------------------------------------
                 List<ControllerBD.SQLParametro> parametro = new List<ControllerBD.SQLParametro>() 
                { 
                    new ControllerBD.SQLParametro("@nomeCod",  nameProd),
                    ...
                    new ControllerBD.SQLParametro("@quant",  this.QUANTIDADE_ENTRADA),
                    new ControllerBD.SQLParametro("@preco",  this.PRECO_UNITARIO),
                    new ControllerBD.SQLParametro("@codigo",  this.CODIGO_BARRA)
         ...
         {
             var query1 = "SELECT Nome_Medicamento as Medicamento,Quantidade_Entrada as Quantidade,Data_Validade  as Validade,Preco_Unitario as Preço FROM tbProduto";
             return new ControllerBD().EXECUTE_READER(query1);
         }

         public DataTable ListarProdutosExpirados(DateTime data)
         {
             var query1 = "SELECT Nome_Medicamento as Medicamento,Quantidade_Entrada as Quantidade,Data_Validade  as Validade,Preco_Unitario as Preço, Unidade as Unidade FROM tbProduto where Data_Validade<=@data ";
             List<ControllerBD.SQLParametro> parametro1 = new List<ControllerBD.SQLParametro>() 
                { 
                    new ControllerBD.SQLParametro("@data",  data.AddMonths(1))
                };
             return new ControllerBD().EXECUTE_READER(query1,parametro1);
         }


         public DataTable ListarProdutosArmazem2()
         {
             var query1 = "SELECT Nome_Medicamento as Medicamento,Quantidade_Entrada as Quantidade,Data_Validade  as Validade,Preco_Unitario as Preço,Unidade FROM tbProduto";
             return new ControllerBD().EXECUTE_READER(query1);
         }

         public DataTable ListarProdutosRelatorioArmazem2()
         {
             var query1 = "SELECT Nome_Medicamento as Medicamento,Preco_Unitario as Preço,Data_Validade  as Validade,Unidade FROM tbProduto";
             return new ControllerBD().EXECUTE_READER(query1);
         }

         public DataTable ListarProdutosRelatorioFinal()
         {
             var query1 = "SELECT Nome_Medicamento as Medicamento,Preco_Unitario as Preço FROM tbProduto order by  Medicamento asc";
             return new ControllerBD().EXECUTE_READER(query1);
         }

         public DataTable ListarProdutosArmazem1()
         {
             var query1 = "SELECT Nome_Medicamento as Medicamento,Quantidade_Entrada as Quantidade,Data_Validade  as Validade,Preco_Unitario as Preço, Unidade FROM tbProduto WHERE Nome_Medicamento Like @nom";
             //------------------------------------------------------------------------------
             List<ControllerBD.SQLParametro> parametro = new List<ControllerBD.SQLParametro>() 
                {
         ...
         public void Analizar()
         {
             #region pegar a quantidade anterior
                        int d = QuantidadeInserir(this.NOME_MEDICAMENTO);
                         d += QUANTIDADE_OUTRO;
        ...
        //========================================================
         public int QuantidadeInserir(string NOME_MEDICAMENTO) {
             //------------------------------------------------------------------------------
         ...
         public static int SelectTaxaIVA(string produto)
         {
             //------------------------------------------------------------------------------
             var query1 = "SELECT A.Taxa FROM tbProduto Inner Join tbAGT as A on tbProduto.idAgt=A.id WHERE Nome_Medicamento=@idproduto";
             //------------------------------------------------------------------------------
             List<ControllerBD.SQLParametro> parametro1 = new List<ControllerBD.SQLParametro>() 
                { 
                    new ControllerBD.SQLParametro("@idproduto",  produto)
         ...
         public List<string> ListarProdutos(string produto,int qtd)
         {
             ...
             var Dados = from dado in tbLogin.AsEnumerable()
                         select new
                         {
                             ...
                             //Total = dado.Field<int>("Preco_Unitario") //dado.Field<string>("Unidade")
                         };
             ...
             foreach (var item in Dados)
             {
                 ...
                 lista.Add((item.Preco*qtd).ToString()); //item.Unidade.ToString()
             }
             return lista;
             //======================================================================================
         }
