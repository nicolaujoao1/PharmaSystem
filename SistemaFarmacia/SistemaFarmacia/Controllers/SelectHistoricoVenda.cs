    ...
    public class SelectHistoricoVenda
    {
        public static DataTable CarregarDadosHistoricos(int id)
        {
            try
            {
                ...
                var query = "SELECT V.Nome_Cliente AS Cliente,V.Quantidade,V.Valor_Pago,F.Nome AS Funcionario,V.Tipo_Pagamento ,V.Data_Venda FROM tbVenda AS V INNER JOIN tbFuncionario AS F ON F.id=V.ID_Funcionario WHERE V.Data_Venda=@data AND V.ID_Funcionario=@id";
                //------------------------------------------------------------------------------
